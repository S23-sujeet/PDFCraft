...
A free, client-side PDF tools website (merge, split, compress, rotate, watermark,
page numbers, images <-> PDF) built with React + TypeScript + Vite. All file
processing runs entirely in the browser via `pdf-lib` and `pdf.js` - no backend,
no file uploads, no server storage. That makes it a static site: cheap, fast,
and trivial to deploy anywhere.

## Name & domain

The site is branded **PDFCraft**, using `pdfcraft.app` as the placeholder
domain throughout this repo's SEO config (`index.html`, `robots.txt`,
`sitemap.xml`, `.env.example`). Other options considered, in case `pdfcraft.app`
turns out to be unavailable:

| Priority | Name | Domain (verify availability before buying) | Why |
| --- | --- | --- | --- |
| 1 | **PDFCraft** | pdfcraft.app / pdfcraft.io | Implies quality/craftsmanship, easy to say, brandable, keeps "PDF" for search relevance. Current brand for this repo. |
| 2 | **PDFBolt** | pdfbolt.app / pdfbolt.com | Short, memorable, implies speed. |
| 3 | **QuickPDF Hub** | quickpdfhub.com | Very keyword-forward (good for exact-match search intent), less unique as a brand. |
| 4 | **Foldr** | foldr.app | Highly brandable, memorable, .app TLD signals "tool" - weaker on the "pdf" keyword though, would need "Foldr - PDF Tools" in titles to compensate. |
| 5 | **PDFNest** | pdfnest.com | Friendly, short, keyword-relevant. |
...
4. Environment variables (Pages project settings -> **Environment variables**), optional:
   - `VITE_SITE_URL` = your real domain, e.g. `https://www.pdfcraft.app`
...
npm install
npm run build
npx wrangler pages deploy dist --project-name=pdfcraft
```
...
docker build -t pdfcraft .
docker run -d --name pdfcraft -p 80:80 --restart unless-stopped pdfcraft
```
...
scp -r dist/* user@your-server:/var/www/pdfcraft/
```
